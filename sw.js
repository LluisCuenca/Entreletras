const CACHE='entreletras-v1';
const ASSETS=['./','./index.html','./style.css','./puzzles.js','./app.js','./icon.svg','./icon-192.png','./icon-512.png','./manifest.webmanifest'];
self.addEventListener('install',event=>{event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(ASSETS)));self.skipWaiting();});
self.addEventListener('activate',event=>{event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('entreletras-')&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim()));});
